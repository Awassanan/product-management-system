    ...
    await fetch(baseURL)
        ...
        .then(data => {
            ...
            data.forEach(element => {
                document.getElementById("tbody").innerHTML += `<tr id="${"product" + element.id}">
                <th scope="row" id="${"row" + element.id}">${element.id}</th>
                <td>
                    <div class="d-flex justify-content-between" style="width:60%">
                        <i class="bi-pencil-square" onclick='updateProductModal(${element.id}, "${element.name}", "${element.category}", ${element.price}, ${element.stock})'></i>
                        <i class="bi-trash-fill" onclick="deleteProduct('${element.id}')"></i>
                    </div>
                </td>
                <td>${element.name}</td>
...
async function searchById(id) {
    ...
    await fetch(baseURL + id)
        ...
        .then(data => {
            ...
            document.getElementById("tbody").innerHTML = `<tr id="${"product" + data.id}">
                <th scope="row" id="${"row" + data.id}">${data.id}</th>
                <td>
                    <div class="d-flex justify-content-between" style="width:60%">
                        <i class="bi-pencil-square" onclick='updateProductModal(${data.id}, "${data.name}", "${data.category}", ${data.price}, ${data.stock})'></i>
                        <i class="bi-trash-fill" onclick="deleteProduct('${data.id}')"></i>
                    </div>
                </td>
                <td>${data.name}</td>
                <td>${data.category}</td>
...
async function updateProduct(updateData) {
    await fetch(baseURL + updateData.id, {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json'
        },
        ...
        .then(data => {
            ...
            document.getElementById(`product${data.id}`).innerHTML = `<th scope="row" id="row${data.id}">${data.id}</th>
            <td>
                <div class="d-flex justify-content-between" style="width:60%">
                    <i class="bi-pencil-square" onclick='updateProductModal(${data.id}, "${data.name}", "${data.category}", ${data.price}, ${data.stock})'></i>
                    <i class="bi-trash-fill" onclick="deleteProduct('${data.id}')"></i>
